...
import { getUserPurchaseAreas } from 'redux/actions/Users';
import { useDebounce } from '@jumbo/utils/commonHelper';

import TablePagination from '@material-ui/core/TablePagination';
import NoRecordFound from '../NoRecordFound';

import UserPurchaseAreasTableHead from '../UserPurchaseAreasTableHead';
import UserPurchaseAreasTableToolbar from '../UserPurchaseAreasTableToolbar';
import UserPurchaseAreasListRow from '../UserPurchaseAreasListRow';
...
import { addUserPurchaseAreas } from 'redux/actions/Users';
...
const UserPurchaseAreas = ({ open, onCloseDialog }) => {
  ...
  const [userPurchaseAreasFetched, setUserPurchaseAreasFetched] = useState(false);

  const { currentUser, currentUserPurchaseAreas } = useSelector(({ usersReducer }) => usersReducer);
  ...
  useEffect(
    () => {
      ...
      dispatch(
        getUserPurchaseAreas(currentUser.userId, filterOptions, debouncedSearchTerm, () => {
          setFilterApplied(!!filterOptions.length || !!debouncedSearchTerm);
          setUserPurchaseAreasFetched(true);
        }),
      );
    },
  ...
  /*   useEffect(() => {
    getLoadUserRolesSelected(currentUserPurchaseAreas);
  }, [currentUserPurchaseAreas]);  */
  ...
  const handleSelectAllClick = event => {
    if (event.target.checked) {
      const newSelected = currentUserPurchaseAreas.map(n => n.uspaPurcCode);
  ...
  const userRolesReq = await fetch(`${process.env.REACT_APP_API_URL}v1/userRoles/getAllRolesByUserId?uspaUserId=${userId}`, 
  { 
    method: 'get', 
    headers: new Headers({
  ...
  /*   const getLoadUserRolesSelected = (array) => {
    ...
    for (let i = 0; i<array.length; i++){
      obj['uspaPurchaseCode']=array[i].uspaPurchaseCode;
      obj['lecturas']=0;
  ...
  /*   const isSelected = (id) => { 
    ...
    for (let i = 0; i<loadtUserRolesSelected.length; i++){
      if ( loadtUserRolesSelected[i].uspaPurchaseCode === id && loadtUserRolesSelected[i].lecturas === 0 && loadtUserRolesSelected[i].asigned ==='S' ){
        obj['uspaPurchaseCode']=loadtUserRolesSelected[i].uspaPurchaseCode;
        obj['lecturas']=1;
  ...
  const handleConfirmCreateRoles = () => {
    ...
    let userPurchaseAreas = [];
    let obj = {};
    for (let i = 0; i < selected.length; i++) {
      obj = {};
      obj['uspaUserId'] = currentUser.userId;
      obj['uspaPurcCode'] = selected[i];
      obj['uspaSelected'] = 'S';
      obj['uspaStatus'] = 'S';
      userPurchaseAreas.push(obj);
      //userRoles.name='userRoles';
      //console.log(selected[i]);
    }

    dispatch(addUserPurchaseAreas(userPurchaseAreas, () => {}));
    ...
    setTimeout(() => {
      dispatch(fetchSuccess(<IntlMessages id="fetch.user.delete.purchaseAreasAssignment.message" />));
    }, 2000);
  ...
  const onSubmitClick = () => {
    if (selected.length === 0) {
      ...
      dispatch(fetchError(<IntlMessages id="users.appModule.purchaseAreasAssignment.emptyPurchaseAreas" />));
    } else {
  ...
  return (
    <Dialog open={open} onClose={onCloseDialog} className={classes.dialogRoot}>
      <DialogTitle className={classes.dialogTitleRoot}>
        <IntlMessages id="users.userPurchaseAreas.purchaseAreaAssignment.form.editTitle" />
      </DialogTitle>
      <DialogContent dividers>
        ...
        <Paper className={classes.paper}>
          ...
          <TableContainer className={classes.container}>
            <Table stickyHeader className={classes.table} aria-labelledby="tableTitle" aria-label="sticky enhanced table">
              <UserPurchaseAreasTableHead
                ...
                rowCount={currentUserPurchaseAreas.length}
              />
              <TableBody>
                {!!currentUserPurchaseAreas.length ? (
                  stableSort(currentUserPurchaseAreas, getComparator(order, orderBy))
                    .slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage)
                    .map((row, index) => (
                      <UserPurchaseAreasListRow
                        key={index}
                        row={row}
                  ...
                  <TableRow style={{ height: 53 * 6 }}>
                    <TableCell colSpan={7} rowSpan={10}>
                      {isFilterApplied ? (
                        ...
                        <NoRecordFound>
                          {userPurchaseAreasFetched ? (
                            <IntlMessages id="users.userPurchaseAreas.label.noRecordsFound" />
                          ) : (
                            <IntlMessages id="users.userPurchaseAreas.label.loadingUsers" />
                          )}
                        </NoRecordFound>
                      )}
                    </TableCell>
          ...
          <TablePagination
            ...
            component="div"
            count={currentUserPurchaseAreas.length}
      ...
      {openUserRolesDialog && <UserRoles open={openUserRolesDialog} onCloseDialog={handleCloseUserRolesDialog} />} */}
        ...
        <ConfirmDialog
          ...
          title={<IntlMessages id="users.appModule.purchaseAreasAssignment.createConfirm" />}
          content={<IntlMessages id="users.appModule.purchaseAreasAssignment.createMessage" />}
...
export default UserPurchaseAreas;

UserPurchaseAreas.prototype = {
