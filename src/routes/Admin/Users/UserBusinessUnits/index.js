...
import { getUserBusinessUnits } from 'redux/actions/Users';
import { useDebounce } from '@jumbo/utils/commonHelper';

import TablePagination from '@material-ui/core/TablePagination';
import NoRecordFound from '../NoRecordFound';

import UserBusinessUnitsTableHead from '../UserBusinessUnitsTableHead';
import UserBusinessUnitsTableToolbar from '../UserBusinessUnitsTableToolbar';
import UserBusinessUnitsListRow from '../UserBusinessUnitsListRow';
...
import { addUserBusinessUnits } from 'redux/actions/Users';
...
const UserBusinessUnits = ({ open, onCloseDialog }) => {
  ...
  const [userBusinessUnitsFetched, setUserBusinessUnitsFetched] = useState(false);

  const { currentUser, currentUserBusinessUnits } = useSelector(({ usersReducer }) => usersReducer);
  ...
  useEffect(
    () => {
      ...
      dispatch(
        getUserBusinessUnits(currentUser.userId, filterOptions, debouncedSearchTerm, () => {
          setFilterApplied(!!filterOptions.length || !!debouncedSearchTerm);
          setUserBusinessUnitsFetched(true);
        }),
      );
    },
  ...
  /*   useEffect(() => {
    getLoadUserRolesSelected(currentUserBusinessUnits);
  }, [currentUserBusinessUnits]);  */
  ...
  const handleSelectAllClick = event => {
    if (event.target.checked) {
      const newSelected = currentUserBusinessUnits.map(n => n.usbuBuCode);
  ...
  const handleConfirmCreateRoles = () => {
    ...
    for (let i = 0; i < selected.length; i++) {
      obj = {};
      obj['usbuUserId'] = currentUser.userId;
      obj['usbuBuCode'] = selected[i];
      obj['usbuSelected'] = 'S';
      obj['usbuStatus'] = 'S';
      userBusinessUnits.push(obj);
      //userRoles.name='userRoles';
      //console.log(selected[i]);
    }

    dispatch(addUserBusinessUnits(userBusinessUnits, () => {}));
    ...
    setTimeout(() => {
      dispatch(fetchSuccess(<IntlMessages id="fetch.user.delete.BusinessUnitsAssignment.message" />));
    }, 2000);
  ...
  const onSubmitClick = () => {
    if (selected.length === 0) {
      ...
      dispatch(fetchError(<IntlMessages id="users.appModule.BusinessUnitsAssignment.emptyBusinessUnits" />));
    } else {
  ...
  return (
    <Dialog open={open} onClose={onCloseDialog} className={classes.dialogRoot}>
      <DialogTitle className={classes.dialogTitleRoot}>
        <IntlMessages id="users.userBusinessUnits.BusinessUnitAssignment.form.editTitle" />
      </DialogTitle>
      <DialogContent dividers>
        ...
        <Paper className={classes.paper}>
          ...
          <TableContainer className={classes.container}>
            <Table stickyHeader className={classes.table} aria-labelledby="tableTitle" aria-label="sticky enhanced table">
              <UserBusinessUnitsTableHead
                ...
                rowCount={currentUserBusinessUnits.length}
              />
              <TableBody>
                {!!currentUserBusinessUnits.length ? (
                  stableSort(currentUserBusinessUnits, getComparator(order, orderBy))
                    .slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage)
                    .map((row, index) => (
                      <UserBusinessUnitsListRow
                        key={index}
                        row={row}
                  ...
                  <TableRow style={{ height: 53 * 6 }}>
                    <TableCell colSpan={7} rowSpan={10}>
                      {isFilterApplied ? (
                        ...
                        <NoRecordFound>
                          {userBusinessUnitsFetched ? (
                            <IntlMessages id="users.userBusinessUnits.label.noRecordsFound" />
                          ) : (
                            <IntlMessages id="users.userBusinessUnits.label.loadingUsers" />
                          )}
                        </NoRecordFound>
                      )}
                    </TableCell>
          ...
          <TablePagination
            ...
            component="div"
            count={currentUserBusinessUnits.length}
      ...
      {openUserRolesDialog && <UserRoles open={openUserRolesDialog} onCloseDialog={handleCloseUserRolesDialog} />} */}
        ...
        <ConfirmDialog
          ...
          title={<IntlMessages id="users.appModule.BusinessUnitsAssignment.createConfirm" />}
          content={<IntlMessages id="users.appModule.BusinessUnitsAssignment.createMessage" />}
...
export default UserBusinessUnits;

UserBusinessUnits.prototype = {
