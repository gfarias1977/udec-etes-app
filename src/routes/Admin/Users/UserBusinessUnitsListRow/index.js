...
const UserBusinessUnitsListRow = ({
  row,
  isSelected,
  onRowClick,
  onUserEdit,
  ...
  const labelId = `enhanced-table-checkbox-${row.usbuBuCode}`;
  const isItemSelected = isSelected(row.usbuBuCode);
  ...
  return (
    <TableRow
      hover
      onClick={event => onRowClick(event, row.usbuBuCode)}
      role="checkbox"
      ...
      <TableCell>{row.usbuBuName}</TableCell>
...
export default React.memo(UserBusinessUnitsListRow);
