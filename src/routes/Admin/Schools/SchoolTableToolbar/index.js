...
const filterOptionsList = [
  {
    label: <IntlMessages id="schools.appModule.filterActive" />,
    value: 'S',
  },
  {
    label: <IntlMessages id="schools.appModule.filterInactive" />,
    value: 'N',
  },
];

const SchoolTableToolbar = ({
  selected,
  setSelected,
  onSchoolAdd,
  filterOptions,
  ...
  return (
    <React.Fragment>
      <Toolbar
        ...
        {numSelected > 0 ? (
          <Typography className={classes.title} color="inherit" variant="subtitle1" component="div">
            {numSelected}{' '}
            {numSelected > 1 ? (
              <IntlMessages id="schools.appModule.selectedSchools" />
            ) : (
              <IntlMessages id="schools.appModule.selectedSchool" />
            )}
          </Typography>
        ) : (
          <Typography className={classes.title} variant="h4" id="tableTitle" component="div">
            {<IntlMessages id="schools.appModule.schools" />}{' '}
            <Button color="primary" onClick={() => onSchoolAdd(true)}>
              {<IntlMessages id="schools.appModule.addNewSchool" />}
            </Button>
          </Typography>
        )}

        {numSelected > 0 ? (
          <Tooltip title={<IntlMessages id="schools.appModule.deleteSchool" />}>
          ...
          <React.Fragment>
            ...
            <Tooltip title={<IntlMessages id="schools.appModule.filterList" />}>
      ...
      <ConfirmDialog
        ...
        title={<IntlMessages id="schools.appModule.confirmDelete" />}
        content={<IntlMessages id="schools.appModule.confirmDeleteContent" />}
...
export default React.memo(SchoolTableToolbar);
