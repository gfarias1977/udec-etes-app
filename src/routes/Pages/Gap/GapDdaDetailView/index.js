...
import { getAllGapPeriodsDda, getAllGapDemandVsStock } from 'redux/actions/Gap';
...
import ExportButtonGapsDemandVsStockReport from 'components/ExportButtonGapsDemandVsStockReport';

const GapDdaDetailViewModule = ({ open, onCloseDialog }) => {
  const classes = useStyles();
  const { gapsDda, gapsPeriodsDda } = useSelector(({ gapsReducer }) => gapsReducer);
  ...
  const handleSelectAllClick = event => {
    if (event.target.checked) {
      const newSelected = gapsDda.map(n => n.procId);
          ...
          <Paper className={classes.paper}>
            <Card className={classes.root}>
              <CardHeader
                ...
                title={
                  <IntlMessages
                    id="gaps.gapsDetailview.labelProcCode.title"
                ...
                <Box display="flex" flexDirection={{ xs: 'column', md: 'row' }} alignItems="center" mb={{ xs: 6, md: 5 }}>
                  <GridContainer spacing={2}>
                    ...
                    <Grid item xs={12} sm={6}>
                      ...
                      <Box display="flex" justifyContent="flex-end" mb={4}>
                        <Box ml={2}>
                          {gapsDda && gapsDda.length > 0 && <ExportButtonGapsDemandVsStockReport data={gapsDda} />}
                        </Box>
                      </Box>
                    </Grid>
                  </GridContainer>
            ...
            <TableContainer className={classes.container}>
              <Table stickyHeader className={classes.table} aria-labelledby="tableTitle" aria-label="sticky enhanced table">
                <GapDetailViewTableHead
                  ...
                  rowCount={gapsDda.length}
                />
                <TableBody>
                  {!!gapsDda.length ? (
                    stableSort(gapsDda, getComparator(order, orderBy))
